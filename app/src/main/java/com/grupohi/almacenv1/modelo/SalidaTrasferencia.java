...
public class SalidaTrasferencia {
	DB almacen;
	SQLiteDatabase db;
	final String log = "";
	Context conthis;

	public SalidaTrasferencia(Context context) {
		almacen = new DB(context);
		conthis=context;
	}
	
	public List<ListviewGenerico> getListViewSalidaTrasferencia(Context context) {
		List<ListviewGenerico> labels = new ArrayList<ListviewGenerico>();
		
		String selectQuery = "SELECT SC.*,  A."+DB.COLUMN_DESCRIPCION+" FROM "
				+ DB.TABLE_SALIDAS_TRANSFERENCIA+ " SC "+ 
	...
	public List<ListviewGenerico> SalidaConsumoMateriales(String clavesalida) {
		...
		String selectQuery = "SELECT SFP.*, M."+DB.COLUMN_DESCRIPCION+", A."+DB.COLUMN_DESCRIPCION+" desal, C."+DB.COLUMN_DESCRIPCION+" as nombrecontratista  FROM "
				+ DB.TABLE_SALIDAS_TRANSFERENCIA_PARTIDADAS+ " SFP "
				+ " LEFT JOIN "+DB.TABLE_MATERIALES+" M "+" ON  M."+DB.COLUMN_IDMATERIAL+" = SFP."+DB.COLUMN_IDMATERIAL
				+ " LEFT JOIN "+DB.TABLE_ALMACEN+" A "+" ON  A."+DB.COLUMN_IDALMACEN+" = SFP."+DB.COLUMN_IDALMACEN
				+ " LEFT JOIN "+DB.TABLE_CONTRATISTAS+" C "+" ON  C."+DB.COLUMN_IDCONTRATISTA+" = SFP."+DB.COLUMN_IDCONTRATISTA
				+ " WHERE SFP."+ DB.COLUMN_CLAVESALIDA+" = \""+clavesalida+"\"";
		Log.d(log, selectQuery);
		db = almacen.getReadableDatabase();
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor.moveToFirst()) {
			do {
				...
				String almacen = cursor.getString(cursor.getColumnIndex("desal"));
				...
				labels.add(new ListviewGenerico(0, nombrematerial, unidad, exist,0, 0, almacen,idcontratista,concargo,nombrecontratista));
			} while (cursor.moveToNext());
	...
	public String getMaterialesSalidaTransferencia(String clave){
	...
	public JSONObject getdata_transferencia(){
		Cursor mCursor;
		...
		try {
			if (db != null) {
				...
				mCursor = db.query(DB.TABLE_SALIDAS_TRANSFERENCIA, columns, null, null, null,
						null, null, null);
	...
	public void deletedata(){
		try {
			...
			db.execSQL("delete from "+ DB.TABLE_SALIDAS_TRANSFERENCIA);
			db.execSQL("delete from "+ DB.TABLE_SALIDAS_TRANSFERENCIA_PARTIDADAS);
	...
	public JSONObject getdata_transferencia_partidas(){
		Cursor mCursor;
		...
		try {
			if (db != null) {
				...
				mCursor = db.query(DB.TABLE_SALIDAS_TRANSFERENCIA_PARTIDADAS, columns, null, null, null,
						null, null, null);
				if (mCursor.moveToFirst()) {
					do {
						JSONObject data=new JSONObject();	
					...
					data.put(DB.COLUMN_IDALMACEN, mCursor.getString(mCursor.getColumnIndex(DB.COLUMN_IDALMACEN)));
